...
export function createTextTool(executor: CliExecutor): ToolDefinition {
  return {
    spec: {
      name: 'chat',
...
function parseTextArgs(params: Record<string, unknown>): string[] {
  const parsed = TextParams.parse(params);
  const args = ['text', 'chat', '--message', parsed.message];

  if (parsed.system) args.push('--system', parsed.system);
  if (parsed.model) args.push('--model', parsed.model);
  if (parsed.max_tokens) args.push('--max-tokens', String(parsed.max_tokens));
  if (parsed.temperature !== undefined) args.push('--temperature', String(parsed.temperature));
  if (parsed.stream) args.push('--stream');
