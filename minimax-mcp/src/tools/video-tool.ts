...
export function createVideoTool(executor: CliExecutor): ToolDefinition {
  return {
    spec: {
      name: 'generate_video',
...
function parseVideoArgs(params: Record<string, unknown>): string[] {
  ...
  if (parsed.first_frame) args.push('--first-frame', parsed.first_frame);
  if (parsed.last_frame) args.push('--last-frame', parsed.last_frame);
  if (parsed.subject_image) args.push('--subject-image', parsed.subject_image);
  if (parsed.download) args.push('--download', parsed.download);
  if (parsed.async) args.push('--async');
  if (parsed.poll_interval) args.push('--poll-interval', String(parsed.poll_interval));
