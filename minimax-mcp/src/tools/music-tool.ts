...
export function createMusicTool(executor: CliExecutor): ToolDefinition {
  return {
    spec: {
      name: 'generate_music',
...
function parseMusicArgs(params: Record<string, unknown>): string[] {
  const parsed = MusicParams.parse(params);
  const args = ['music', 'generate', '--prompt', parsed.prompt];

  if (parsed.lyrics) args.push('--lyrics', parsed.lyrics);
  if (parsed.lyrics_file) args.push('--lyrics-file', parsed.lyrics_file);
  if (parsed.lyrics_optimizer) args.push('--lyrics-optimizer');
  if (parsed.instrumental) args.push('--instrumental');
  if (parsed.vocals) args.push('--vocals', parsed.vocals);
  if (parsed.genre) args.push('--genre', parsed.genre);
  if (parsed.mood) args.push('--mood', parsed.mood);
  if (parsed.instruments) args.push('--instruments', parsed.instruments);
  if (parsed.tempo) args.push('--tempo', parsed.tempo);
  if (parsed.bpm) args.push('--bpm', String(parsed.bpm));
  if (parsed.key) args.push('--key', parsed.key);
