...
export function createSpeechTool(executor) {
    return {
        spec: {
...
function parseSpeechArgs(params) {
    ...
    if (parsed.voice)
        args.push('--voice', parsed.voice);
    if (parsed.model)
        args.push('--model', parsed.model);
    if (parsed.speed !== undefined)
        args.push('--speed', String(parsed.speed));
    if (parsed.volume !== undefined)
        args.push('--volume', String(parsed.volume));
    if (parsed.pitch !== undefined)
        args.push('--pitch', String(parsed.pitch));
    if (parsed.format)
        args.push('--format', parsed.format);
    if (parsed.out)
        args.push('--out', parsed.out);
    if (parsed.language)
        args.push('--language', parsed.language);
    if (parsed.subtitles)
        args.push('--subtitles');
    return args;
}
//# sourceMappingURL=speech-tool.js.map